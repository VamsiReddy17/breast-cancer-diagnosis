...
const AuthRequiredOverlay = ({ onLogin }) => {
  return (
    <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl max-w-lg mx-auto my-12 animate-fadeIn relative overflow-hidden">
      {/* Background gradients */}
      <div className="absolute -top-24 -left-24 w-48 h-48 bg-blue-500/10 dark:bg-blue-500/5 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute -bottom-24 -right-24 w-48 h-48 bg-purple-500/10 dark:bg-purple-500/5 rounded-full blur-3xl pointer-events-none"></div>
      
      <div className="p-4 bg-zinc-50 dark:bg-zinc-900 rounded-full border border-zinc-100 dark:border-zinc-800/80 mb-6 relative">
        <Activity className="w-12 h-12 text-blue-600 dark:text-blue-400 animate-pulse" />
      </div>
      
      <h3 className="text-xl font-bold tracking-tight text-center mb-2">Secure AI Diagnostics Portal</h3>
      <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center max-w-sm mb-8 leading-relaxed">
        Authentication is required to execute real-time model inference. Sign in with Google to query the neural network, run clinical prognosis staging, and register diagnostic logs.
      </p>

      <button
        onClick={onLogin}
        className="w-full flex items-center justify-center gap-3 px-6 py-3 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-950 dark:bg-zinc-50 hover:bg-zinc-900 dark:hover:bg-zinc-100 text-white dark:text-zinc-950 font-bold transition-all shadow-md active:scale-98 cursor-pointer"
      >
...
function App() {
  ...
  const logPredictionAudit = async (type, result) => {
    try {
      ...
      console.warn("Failed to log audit details:", err);
    }
  };
  ...
  const handlePredict = async (e) => {
    ...
    try {
      ...
      // Log prediction audit details in Supabase
      const label = response.data.class_label || (response.data.prediction === 1 ? 'Malignant / High Risk' : 'Benign / Low Risk');
      logPredictionAudit(datasetMode === 'seer' ? 'SEER Clinical Prognosis' : 'Wisconsin Cytology Diagnosis', label);
    } catch (err) {
  ...
  const uploadImageAndPredict = async (e) => {
    ...
    try {
      ...
      // Log prediction audit details in Supabase
      logPredictionAudit('Histopathology Image Classification', response.data.class_label);
    } catch (err) {
      ...
      <header className="border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#0c0c0f] sticky top-0 z-30">
        <div className="max-w-[1600px] mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Auth Profile / Sign In Indicator */}
            {authLoading ? (
              <div className="w-8 h-8 rounded-full border-2 border-zinc-200 dark:border-zinc-800 animate-spin border-t-blue-600"></div>
            ) : user ? (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-zinc-50 dark:bg-zinc-900/50 hover:bg-red-50 dark:hover:bg-red-950/20 hover:border-red-200 dark:hover:border-red-900/50 transition-colors"
                  title="Sign Out"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 border border-zinc-200 dark:border-zinc-800 rounded-lg bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-800 font-bold text-xs transition-colors cursor-pointer"
              >
      ...
      <main className="flex-grow max-w-[1600px] w-full mx-auto p-4 md:p-6">
        ...
        {activeTab === 'home' && (
          <div className="space-y-6 animate-fadeIn max-w-[1000px] mx-auto text-left">
            {!user ? (
              /* Premium SaaS Pre-Auth Landing Page */
              <div className="space-y-12 py-6">
                {/* Hero Section */}
                <div className="text-center space-y-6 max-w-3xl mx-auto relative">
                  <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-72 h-72 bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>
                  
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50/50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/30 text-[10px] uppercase font-bold tracking-widest rounded-full">
                    <Activity className="w-3.5 h-3.5" />
                    Secure Clinical AI Platform
                  </span>
                  ...
                  <p className="text-sm md:text-base text-zinc-500 dark:text-zinc-400 max-w-xl mx-auto leading-relaxed">
                    Empowering oncology research with unified cytology classification, cohort survival prognosis models, and deep learning histopathology image classification.
                  </p>

                  <div className="pt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
                    <button
                      onClick={handleGoogleLogin}
                      className="flex items-center gap-3 px-8 py-3.5 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-950 dark:bg-zinc-50 hover:bg-zinc-900 dark:hover:bg-zinc-100 text-white dark:text-zinc-950 font-bold transition-all shadow-lg active:scale-98 cursor-pointer"
                    >
                      <svg className="w-5 h-5" viewBox="0 0 24 24">
                        <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                        <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6">
                  <div className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm space-y-3 relative overflow-hidden group hover:border-blue-500/30 transition-all duration-300">
                    <div className="p-2.5 bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 rounded-lg w-fit">
                      <Sliders className="w-5 h-5" />
                    </div>
                    <h3 className="text-base font-bold tracking-tight">FNA Cytology Diagnosis</h3>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                      Evaluate cell nuclei shape parameters (radius, perimeter, compactness) with 96.5% accurate KNN, SVM, and Logistic Regression models.
                    </p>
                  </div>

                  <div className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm space-y-3 relative overflow-hidden group hover:border-indigo-500/30 transition-all duration-300">
                    <div className="p-2.5 bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 rounded-lg w-fit">
                      <Database className="w-5 h-5" />
                    </div>
                    <h3 className="text-base font-bold tracking-tight">SEER Cohort Prognosis</h3>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                      Predict mortality probabilities using 4,024 patient clinical staging metrics (AJCC stage, Grade, Estrogen/Progesterone statuses).
                    </p>
                  </div>

                  <div className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm space-y-3 relative overflow-hidden group hover:border-purple-500/30 transition-all duration-300">
                    <div className="p-2.5 bg-purple-50 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400 rounded-lg w-fit">
                      <Camera className="w-5 h-5" />
                    </div>
                    <h3 className="text-base font-bold tracking-tight">Histopathology CNN</h3>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
                      Execute a PyTorch EfficientNet-B0 CNN model directly in the browser to classify biopsy slide images at 97.23% accuracy.
                    </p>
                  </div>
                </div>

                {/* Pipeline Stats Section */}
                <div className="bg-[#fcfcfd] dark:bg-[#0b0b0e] border border-zinc-200/60 dark:border-zinc-800/80 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                  <div className="space-y-1 text-center md:text-left">
                    <h4 className="text-sm font-bold uppercase tracking-wider text-zinc-400 font-mono">Platform Health & Status</h4>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400">All models loaded and cached securely with Supabase audit tracking.</p>
                  </div>
                  <div className="flex flex-wrap justify-center items-center gap-8">
                    <div className="text-center">
                      <div className="text-2xl font-black text-blue-600 dark:text-blue-400">97.23%</div>
                      <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-400">CNN Accuracy</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-black text-indigo-600 dark:text-indigo-400">4,024</div>
                      <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-400">SEER Records</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-black text-purple-600 dark:text-purple-400">0.0889</div>
                      <div className="text-[10px] uppercase font-bold tracking-widest text-zinc-400">Validation Loss</div>
                    </div>
                  </div>
                </div>
              </div>
            ) : (
              /* Standard Logged-In Portal Home Dashboard */
              <>
                {/* Hero Banner */}
                <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white rounded-2xl p-6 md:p-10 shadow-lg relative overflow-hidden">
                  <div className="relative z-10 space-y-4">
                    ...
                    <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
                      {datasetMode === 'seer' ? "OncoSense Prognosis Dashboard" : "OncoSense Diagnosis Dashboard"}
                    </h2>
                    <p className="text-sm md:text-base text-blue-100 max-w-2xl leading-relaxed">
                      {datasetMode === 'seer' 
                        ? "You are logged in. Access clinical survival models, predict risk staging, and browse patient historical profiles using the pipeline tabs above."
                        : "You are logged in. Access FNA cytology models, run live diagnostic metrics, and upload histopathology biopsy images for CNN analysis."}
                    </p>
                  </div>
        ...
        {activeTab === 'inference' && (
          !user ? (
            <AuthRequiredOverlay onLogin={handleGoogleLogin} />
          ) : (
            <div className="space-y-6 animate-fadeIn">
            {/* Page Header */}
            <div>
